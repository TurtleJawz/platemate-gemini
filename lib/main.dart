        ...
        children: [
          Center(
            child: Column(
              ...
              children: [
                Image.asset('lib/assets/icon.png', width: 150, height: 150),
...
class _CameraScreenState extends State<CameraScreen> {
  ...
  Future<void> _takePictureAndProcess() async {
    try {
      ...
      final ingredients = await api.identifyIngredients(image.path);

      if (ingredients.isNotEmpty && ingredients[0].contains("Error")) {
        if (mounted) Navigator.pop(context);
        _showErrorDialog(ingredients[0]);
        return;
      }

      final recipes = await api.fetchRecipes(ingredients);
...
class _RecipeDetailScreenState extends State<RecipeDetailScreen> {
  ...
  Future<void> _loadFullDetails() async {
    final api = ApiService();
    // Use the ID from the initial search to get full info
    final details = await api.fetchRecipeDetails(widget.recipe['id']);
    if (mounted) {
      setState(() {
        fullDetails = details;
        isLoading = false;
